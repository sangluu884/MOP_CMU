...
namespace WindowsFormsApplication.Promotion_Test
{
    class BUS_Promotion
    {
        ...
        public bool deletePromotion(string PromotionID)
        {
            ...
            try
            {
                cmart7.Promotions.DeleteObject(promotion);
                cmart7.SaveChanges();
                flag = true;
            }
            catch (Exception)
        ...
        public bool Insert(string id, float PromotionPrice, DateTime StartDate, string Content, string Image, string ProductID)
        {
            bool flag = false;
            try
            {
                cmart7.SP_PROMOTION_INSERT(PromotionPrice, StartDate, Content, Image, ProductID);
                cmart7.SaveChanges();
                flag = true;
            }
            catch
        ...
        public bool Update(string id, float PromotionPrice, DateTime StartDate, string Content, string Image, string ProductID)
        {
            bool flag = false;
            try
            {
                cmart7.usp_PromotionUpdate(id, PromotionPrice, StartDate, Content, Image, ProductID);
                flag = true;
            }
            catch
            {
